...
from .table_definitions import TABLE_DEFINITION, _TableContent, TABLE_REGISTRY, HEADER_DIRECTION
...
'''
class BusinessCaseWordDocument:
    ...
    def add_input_box(self, word_limit):
...
class InputBox:
    ...
    def add_input_box(self, doc: doc):
        ...
        if self.word_limit != "":
            ...
            r = tbl_footer.add_run(table_footer_word_count.format(self.word_limit))
            r.font.color.rgb = get_general_font_colour_rgb()
            r.font.size = Pt(12)
            r.font.bold = False
            r.font.italic = False
...
class GenericTable():
    ...
    def create_table(self, doc: doc):
        ...
        col_width: float  = _max_table_width / len(tbl.columns)
...
'''
Summary:
    Class for creating a 'checkbox', basically a two-column table with the left hand column as a 'check' column,
...
'''
class CheckBox():
    def __init__(self, check_options):
